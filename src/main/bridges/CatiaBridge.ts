/**
 * CatiaBridge — CATIA V5/V6 Camera Bridge
 * Same pattern as NX/Inventor: launches CatiaBridge.exe, connects via named pipe.
 */
...
export class CatiaBridge implements CadBridge {
  readonly name = 'CATIA';
  ...
  private get exePath(): string {
    const packaged = path.join((process as any).resourcesPath ?? '', 'catia-bridge', 'CatiaBridge.exe');
    if (fs.existsSync(packaged)) return packaged;
    return path.join(__dirname, '..', '..', '..', '..', 'catia-bridge', 'bin', 'CatiaBridge.exe');
  }

  async start(): Promise<void> {
    this._status = 'connecting';
    this.statusCallback?.('connecting', 'Launching CatiaBridge.exe...');

    if (!fs.existsSync(this.exePath)) {
      this._status = 'not-installed';
      this.statusCallback?.('not-installed', `CatiaBridge.exe not found at: ${this.exePath}`);
      return;
    }
    ...
    this.process.stdout?.on('data', (data: Buffer) => {
      for (const line of data.toString().split('\n').filter(Boolean)) {
        if (line.startsWith('STATUS:')) {
          ...
          console.log('[CatiaBridge]', msg);
          if (msg.startsWith('Waiting for CATIA')) this._status = 'connecting';
          else if (msg.startsWith('Connected to CATIA')) {
            this._status = 'live';
            this.statusCallback?.('live');
          }
          else if (msg.startsWith('CATIA disconnected')) this._status = 'connecting';
        }
      }
    });

    this.process.stderr?.on('data', (data: Buffer) => {
      console.error('[CatiaBridge stderr]', data.toString());
    });

    this.process.on('exit', (code) => {
      console.log(`[CatiaBridge] Process exited (code ${code})`);
  ...
  private connectPipe(attempt = 0): void {
    ...
    const PIPE_PATH = '\\\\.\\pipe\\hanomi_catia_camera';
    const sock = net.connect(PIPE_PATH);

    sock.on('connect', () => {
      console.log('[CatiaBridge] Pipe connected');
      this.socket = sock;
      this.buffer = '';
    });
    ...
    sock.on('error', () => {
      ...
      if (attempt < 30) {
        ...
        this._status = 'error';
        this.statusCallback?.('error', 'Could not connect to CATIA bridge pipe');
      }
    });
